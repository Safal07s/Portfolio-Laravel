...
<body>
    <div class="wrapper container mt-4">
        ...
        <form action="{{route('tasks.show',$tasks->id)}}" method="POST" enctype="multipart/form-data" class="row g-3 d-inline " >
          @csrf
          @method('put')
    
                <div class="col-md-4 w-100">
                  <label for="validationCustom01 text" class="form-label fw-bold">Title</label>
                  <input type="text" name="title" class="form-control w-100 " readonly id="validationCustom01" value=" {{$tasks->title}}" required>
                  
                </div>
                <div class="col-md-4 w-100 ">
                  <label for="validationCustom02" class="form-label fw-bold ">Description</label>
                  <textarea name="description" class="form-control" id="validationCustom02" readonly cols="30" rows="3"> {{$tasks->description}}</textarea>
